...
const ContactSection = () => {
  ...
  const inClass = (delay = 'delay-300') =>
    `${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-3'} transition-all duration-700 ${delay}`;
  ...
  return (
    ...
    >
      <div className="flex flex-col lg:flex-row flex-1 min-h-0 overflow-hidden">
        {/* LEFT IMAGE */}
        <div className="w-full lg:w-1/3 flex items-center justify-center p-4 flex-shrink-0">
        ...
        <div
          className="ios-flex-fix w-full lg:w-2/3 flex flex-col flex-1 min-h-0 px-2 mt-4"
          style={{ height: 'calc(var(--safe-vh) * 0.90)' }}
        >
          {/* HEADER (already animates) */}
          <div className={`${inClass('delay-200')}`}>
            <h2
              className={`font-bold uppercase text-lg sm:text-xl md:text-2xl lg:text-xl xl:text-3xl mb-1.5`}
            >
              CONTACTO
            </h2>

            <p className="text-gray-700 text-xs sm:text-sm md:text-base lg:text-sm xl:text-base mb-2">
          ...
          <div className="flex flex-col gap-2 min-h-0 flex-1">
            {/* ROW 1 (two cards) */}
            <div className={`grid grid-cols-1 lg:grid-cols-2 gap-2 flex-1 ${inClass('delay-300')}`}>
              <div className={`bg-[#ecfde9] p-3 rounded-lg flex flex-col justify-center ${inClass('delay-350')}`}>
                <h3 className="font-bold text-center text-sm sm:text-base mb-1.5">Dirección Comercial</h3>
              ...
              <div className={`bg-[#ecfde9] p-3 rounded-lg flex flex-col justify-center ${inClass('delay-400')}`}>
                <h3 className="font-bold text-center text-sm sm:text-base mb-1.5">Noroeste</h3>
                <div className="text-center text-xs sm:text-sm space-y-0.5">
            ...
            {/* ROW 2 */}
            <div className={`bg-[#ecfde9] p-3 rounded-lg flex flex-col justify-center flex-1 ${inClass('delay-450')}`}>
              <h3 className="font-bold text-center text-sm sm:text-base mb-1.5">Altiplano y Sureste</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5 pt-2 text-center text-xs sm:text-sm">
                ...
                <div className="space-y-0.5">
                  ...
                  <p><strong>Email:</strong> omar.mora@ag-kem.com</p>
                </div>
              </div>
            </div>

            {/* ROW 3 WITH MAP */}
            <div className={`bg-[#ecfde9] p-3 rounded-lg flex flex-col md:flex-row gap-3 flex-1 ${inClass('delay-500')}`}>
              <div className={`flex-1 flex flex-col justify-center text-xs sm:text-sm space-y-0.5 ${inClass('delay-550')}`}>
