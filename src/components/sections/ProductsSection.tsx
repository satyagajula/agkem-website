// 'use client';

// import Image from 'next/image';
// import { useRouter } from 'next/navigation';
// import { useEffect, useState } from 'react';

// const ProductsSection = () => {
//   const [isMounted, setIsMounted] = useState(false);

//   useEffect(() => {
//     setIsMounted(true);
//   }, []);
//   const router = useRouter();
//   const products = [
//     {
//       image: '/images/Agkem_product1.png',
//       name: 'Fungicidas',
//       description: 'Opciones para la prevención y control de enfermedades fungosas, con perfiles preventivos/curativos y protección residual.'
//     },
//     {
//       image: '/images/Agkem_product2.png',
//       name: 'Insecticidas',
//       description: 'Alternativas para el manejo de insectos plaga, con diferentes modos de acción y estrategias de rotación para minimizar resistencia.'
//     },
//     {
//       image: '/images/Agkem_product3.png',
//       name: 'Herbicidas',
//       description: 'Soluciones para control de malezas de hoja ancha y gramíneas, con opciones selectivas y no selectivas según cultivo y momento de aplicación.'
//     }
//   ];

//   return (
//     <section className="h-full bg-white overflow-y-auto lg:overflow-hidden">
//       <div className="container-custom h-full flex flex-col">
//         <div className="flex flex-col lg:flex-row gap-2 lg:gap-3 xl:gap-4 2xl:gap-6 3xl:gap-8 4xl:gap-10 min-h-screen lg:min-h-[700px]">
          
//           {/* Left Content Section */}
//           <div className="w-full lg:w-1/2 xl:w-1/2 flex flex-col h-full justify-start lg:py-0 xl:py-0 2xl:py-2">
            
//             <h2
//               className={`font-bold mb-0.5 uppercase
//                          text-lg
//                          sm:text-xl sm:mb-1
//                          md:text-2xl md:mb-1
//                          lg:text-lg lg:mb-0.5
//                          xl:text-2xl xl:mb-0.5
//                          2xl:text-4xl 2xl:mb-3
//                          3xl:text-5xl 3xl:mb-3.5
//                          4xl:text-6xl 4xl:mb-4
//                          transition-opacity duration-500 ${isMounted ? 'opacity-100' : 'opacity-0'}`}
//             >
//               PRODUCTOS
//             </h2>
            
//             <p
//               className={`mb-1 text-gray-700
//                          text-[10px]
//                          sm:text-xs sm:mb-1.5
//                          md:text-sm md:mb-2
//                          lg:text-sm lg:mb-0.5
//                          xl:text-base xl:mb-0.5
//                          2xl:text-lg 2xl:mb-3
//                          3xl:text-xl 3xl:mb-4
//                          4xl:text-2xl 4xl:mb-5
//                          mac-xl:text-2xl mac-xl:mb-8
//                          leading-tight
//                          transition-opacity duration-700 delay-300 ${isMounted ? 'opacity-100' : 'opacity-0'}`}
//             >
//               Nuestro portafolio abarca tres categorías principales para el manejo integral de plagas y enfermedades, con diversos modos de acción para favorecer la rotación y la eficacia.
//             </p>

//             {/* Image for mobile/tablet */}
//             <div className="lg:hidden w-full flex justify-center mb-2 sm:mb-3">
//               <div className="relative w-full max-w-md aspect-[4/3]">
//                 <Image
//                   src="/images/Agkem_productBG.png"
//                   alt="Products Background"
//                   fill
//                   className="object-cover rounded-lg"
//                   sizes="(max-width: 768px) 90vw, 50vw"
//                 />
//               </div>
//             </div>
            
//             {/* Products List */}
//             <div
//               className={"flex flex-col gap-2 flex-grow"}
//             >
//               {products.map((product, index) => (
//                 <div
//                   key={index}
//                   className="flex flex-col items-start text-left p-0"
//                   data-testid={`product-card-${index + 1}`}
//                 >
//                   <div
//                     className="relative cursor-pointer mb-0.5 lg:mb-0.5 xl:mb-0.5
//                                w-16 h-10
//                                sm:w-18 sm:h-11
//                                md:w-20 md:h-12
//                                lg:w-20 lg:h-10
//                                xl:w-28 xl:h-14
//                                2xl:w-48 2xl:h-24
//                                mac:w-52 mac:h-26
//                                mac-xl:w-56 mac-xl:h-28
//                                3xl:w-56 3xl:h-28
//                                4xl:w-64 4xl:h-32"
//                     onClick={() => router.push(`/productos/${index + 1}`)}
//                   >
//                     <Image
//                       src={product.image}
//                       alt={product.name}
//                       fill
//                       className="object-contain"
//                       sizes="(max-width: 640px) 64px, (max-width: 768px) 72px, (max-width: 1024px) 80px, (max-width: 1280px) 112px, (max-width: 1440px) 208px, (max-width: 1680px) 224px, (max-width: 1920px) 224px, 256px"
//                       priority={index === 0}
//                     />
//                   </div>
//                   <p className="text-gray-600 mb-1 leading-tight
//                                text-[9px]
//                                sm:text-[9px]
//                                md:text-xs
//                                lg:text-sm
//                                xl:text-sm
//                                2xl:text-lg
//                                mac:text-xl
//                                mac-xl:text-2xl
//                                mac-xl:mb-8
//                                3xl:text-xl
//                                4xl:text-2xl
//                                max-w-full">
//                     {product.description}
//                   </p>
//                 </div>
//               ))}
//             </div>
//           </div>
          
//           {/* Right Image Section - Desktop only */}
//           <div className="hidden lg:flex lg:w-1/2 xl:w-1/2 items-start justify-end lg:pr-4 xl:pr-6 2xl:pr-8">
//             <div className="relative w-full max-w-[450px] aspect-square lg:aspect-[3/4]">
//               <Image
//                 src="/images/Agkem_productBG.png"
//                 alt="Products Background"
//                 fill
//                 className="object-contain"
//                 sizes="(max-width: 1280px) 300px, (max-width: 1536px) 440px, (max-width: 1920px) 560px, 680px"
//               />
//             </div>
//           </div>
//         </div>
//       </div>
//     </section>
//   );
// };

// export default ProductsSection;



// 'use client';

// import Image from 'next/image';
// import { useRouter } from 'next/navigation';
// import { useEffect, useState } from 'react';

// const ProductsSection = () => {
//   const router = useRouter();
//   const [mounted, setMounted] = useState(false);

//   useEffect(() => setMounted(true), []);

//   const products = [
//     {
//       image: '/images/Agkem_product1.png',
//       name: 'Fungicidas',
//       description:
//         'Opciones para la prevención y control de enfermedades fungosas, con perfiles preventivos/curativos y protección residual.',
//     },
//     {
//       image: '/images/Agkem_product2.png',
//       name: 'Insecticidas',
//       description:
//         'Alternativas para el manejo de insectos plaga, con diferentes modos de acción y estrategias de rotación para minimizar resistencia.',
//     },
//     {
//       image: '/images/Agkem_product3.png',
//       name: 'Herbicidas',
//       description:
//         'Soluciones para control de malezas de hoja ancha y gramíneas, con opciones selectivas y no selectivas según cultivo y momento de aplicación.',
//     },
//   ];

//   return (
//     <section className="bg-white w-full py-10">
//       <div className="container-custom mx-auto px-4">

//         {/* Title */}
//         <h2
//           className={`font-bold uppercase text-[#2C3E2C]
//           text-[26px] leading-tight mb-3
//           transition-opacity duration-500
//           ${mounted ? 'opacity-100' : 'opacity-0'}`}
//         >
//           PRODUCTOS
//         </h2>

//         {/* Intro Paragraph */}
//         <p
//           className={`text-[#555] text-[15px] leading-snug max-w-[700px] mb-6
//           transition-opacity duration-700 delay-200
//           ${mounted ? 'opacity-100' : 'opacity-0'}`}
//         >
//           Nuestro portafolio abarca tres categorías principales para el manejo integral 
//           de plagas y enfermedades, con diversos modos de acción para favorecer 
//           la rotación y la eficacia.
//         </p>

//         {/* MAIN WRAPPER — forces desktop layout everywhere */}
//         <div className="flex flex-row w-full items-start justify-between gap-6 
//                         h-auto">

//           {/* LEFT SIDE CONTENT — fixed spacing EXACTLY like PDF */}
//           <div className="flex flex-col flex-shrink-0 w-[55%] min-w-[55%]">

//             {products.map((p, i) => (
//               <div key={i} className="mb-6">
//                 {/* product icon */}
//                 <div
//                   className="relative w-[180px] h-[70px] cursor-pointer mb-1"
//                   onClick={() => router.push(`/productos/${i + 1}`)}
//                 >
//                   <Image
//                     src={p.image}
//                     alt={p.name}
//                     fill
//                     className="object-contain"
//                     priority={i === 0}
//                   />
//                 </div>

//                 {/* description */}
//                 <p className="text-[#444] text-[15px] leading-snug max-w-[700px]">
//                   {p.description}
//                 </p>
//               </div>
//             ))}
//           </div>

//           {/* RIGHT IMAGE — fixed to match PDF EXACTLY */}
//           <div className="flex w-[45%] justify-end items-start">

//             <div className="
//               relative 
//               min-h-[650px]
//               h-[650px]
//               w-full 
//               max-w-[520px]
//             ">
//               <Image
//                 src="/images/Agkem_productBG.png"
//                 alt="Products background collage"
//                 fill
//                 className="object-contain"
//                 sizes="520px"
//               />
//             </div>

//           </div>
//         </div>
//       </div>
//     </section>
//   );
// };

// export default ProductsSection;


// 'use client';

// import Image from 'next/image';
// import { useRouter } from 'next/navigation';
// import { useEffect, useState } from 'react';

// const ProductsSection = () => {
//   const router = useRouter();
//   const [mounted, setMounted] = useState(false);

//   useEffect(() => setMounted(true), []);

//   const products = [
//     {
//       image: '/images/Agkem_product1.png',
//       name: 'Fungicidas',
//       description:
//         'Opciones para la prevención y control de enfermedades fungosas, con perfiles preventivos/curativos y protección residual.',
//     },
//     {
//       image: '/images/Agkem_product2.png',
//       name: 'Insecticidas',
//       description:
//         'Alternativas para el manejo de insectos plaga, con diferentes modos de acción y estrategias de rotación para minimizar resistencia.',
//     },
//     {
//       image: '/images/Agkem_product3.png',
//       name: 'Herbicidas',
//       description:
//         'Soluciones para control de malezas de hoja ancha y gramíneas, con opciones selectivas y no selectivas según cultivo y momento de aplicación.',
//     },
//   ];

//   return (
//     <section className="bg-white w-full py-10 -mt-10">
//       <div className="container-custom mx-auto px-4">

//         {/* Title */}
//         <h2
//           className={`font-bold uppercase text-[#2C3E2C]
//           text-[26px] leading-tight mb-3
//           transition-opacity duration-500
//           ${mounted ? 'opacity-100' : 'opacity-0'}`}
//         >
//           PRODUCTOS
//         </h2>

//         {/* Intro Paragraph */}
//         <p
//           className={`text-[#555] text-[17px] leading-snug max-w-[700px] mb-6
//           transition-opacity duration-700 delay-200
//           ${mounted ? 'opacity-100' : 'opacity-0'}`}
//         >
//           Nuestro portafolio abarca tres categorías principales para el manejo integral 
//           de plagas y enfermedades, con diversos modos de acción para favorecer 
//           la rotación y la eficacia.
//         </p>

//         {/* FULL DESKTOP LAYOUT FORCED ALWAYS */}
//         <div className="flex flex-row w-full items-start justify-between gap-6">

//           {/* LEFT COLUMN */}
//           <div className="flex flex-col flex-shrink-0 w-[55%] min-w-[55%]">

//             {products.map((p, i) => (
//               <div key={i} className="mb-6">
//                 {/* product icon */}
//                 <div
//                   className="relative w-[180px] h-[70px] cursor-pointer mb-1"
//                   onClick={() => router.push(`/productos/${i + 1}`)}
//                 >
//                   <Image
//                     src={p.image}
//                     alt={p.name}
//                     fill
//                     className="object-contain"
//                     priority={i === 0}
//                   />
//                 </div>

//                 {/* Product description text */}
//                 <p className="text-[#444] text-[17px] leading-snug max-w-[700px]">
//                   {p.description}
//                 </p>
//               </div>
//             ))}
//           </div>

//           {/* RIGHT IMAGE COLUMN (TOP-ALIGNED ALWAYS) */}
//           <div className="flex w-[45%] justify-end items-start">
//             <div
//               className="
//                 relative 
//                 w-full 
//                 max-w-[520px]
//                 h-full
//                 min-h-[550px]
                
//                 /* Forces image to match left column height */
//                 self-start 
//                 flex-grow
//               "
//             >
//               <Image
//                 src="/images/Agkem_productBG.png"
//                 alt="Products background collage"
//                 fill
//                 className="object-contain"
//                 sizes="520px"
//               />
//             </div>
//           </div>

//         </div>
//       </div>
//     </section>
//   );
// };

// export default ProductsSection;



// 'use client';

// import Image from 'next/image';
// import { useRouter } from 'next/navigation';
// import { useEffect, useState } from 'react';

// const ProductsSection = () => {
//   const router = useRouter();
//   const [mounted, setMounted] = useState(false);

//   useEffect(() => setMounted(true), []);

//   const products = [
//     {
//       image: '/images/Agkem_product1.png',
//       name: 'Fungicidas',
//       description:
//         'Opciones para la prevención y control de enfermedades fungosas, con perfiles preventivos/curativos y protección residual.',
//     },
//     {
//       image: '/images/Agkem_product2.png',
//       name: 'Insecticidas',
//       description:
//         'Alternativas para el manejo de insectos plaga, con diferentes modos de acción y estrategias de rotación para minimizar resistencia.',
//     },
//     {
//       image: '/images/Agkem_product3.png',
//       name: 'Herbicidas',
//       description:
//         'Soluciones para control de malezas de hoja ancha y gramíneas, con opciones selectivas y no selectivas según cultivo y momento de aplicación.',
//     },
//   ];

//   return (
//     /* wrapper is relative so the right-collage can be absolute inside it */
//     <section className="bg-white w-full py-10 relative overflow-visible">
//       <div className="container-custom mx-auto px-4 relative">

//         {/* Title */}
//         <h2
//           className={`font-bold uppercase text-[#2C3E2C]
//           text-[26px] leading-tight mb-3
//           transition-opacity duration-500
//           ${mounted ? 'opacity-100' : 'opacity-0'}`}
//         >
//           PRODUCTOS
//         </h2>

//         {/* Intro Paragraph (bumped to 17px) */}
//         <p
//           className={`text-[#555] text-[17px] leading-snug max-w-[700px] mb-6
//           transition-opacity duration-700 delay-200
//           ${mounted ? 'opacity-100' : 'opacity-0'}`}
//         >
//           Nuestro portafolio abarca tres categorías principales para el manejo integral 
//           de plagas y enfermedades, con diversos modos de acción para favorecer 
//           la rotación y la eficacia.
//         </p>

//         {/* MAIN: left column stays in flow; right collage absolutely positioned */}
//         <div className="relative w-full">

//           {/* LEFT column: same visual design, width preserved */}
//           <div className="flex flex-col w-[55%] min-w-[55%]">
//             {products.map((p, i) => (
//               <div key={i} className="mb-6">
//                 <div
//                   className="relative w-[180px] h-[70px] cursor-pointer mb-1"
//                   onClick={() => router.push(`/productos/${i + 1}`)}
//                 >
//                   <Image
//                     src={p.image}
//                     alt={p.name}
//                     fill
//                     className="object-contain"
//                     priority={i === 0}
//                   />
//                 </div>
//                 <p className="text-[#444] text-[17px] leading-snug max-w-[700px]">
//                   {p.description}
//                 </p>
//               </div>
//             ))}
//           </div>

//           {/* RIGHT collage: absolute, top-aligned to the container */}
//           <div
//             className="absolute right-0 top-0 w-[45%] pointer-events-none"
//             style={{ transformOrigin: 'top right' }}
//           >
//             <div
//               className="relative w-full max-w-[520px] h-auto"
//               /* use a fixed min-height that matches your PDF's visual proportion */
//               style={{ minHeight: 650 }}
//             >
//               <Image
//                 src="/images/Agkem_productBG.png"
//                 alt="Products background collage"
//                 fill
//                 className="object-contain"
//                 sizes="520px"
//               />
//             </div>
//           </div>
//         </div>
//       </div>
//     </section>
//   );
// };

// export default ProductsSection;


// 'use client';

// import Image from 'next/image';
// import { useRouter } from 'next/navigation';
// import { useEffect, useState } from 'react';

// const ProductsSection = () => {
//   const router = useRouter();
//   const [mounted, setMounted] = useState(false);

//   useEffect(() => setMounted(true), []);

//   const products = [
//     {
//       image: '/images/Agkem_product1.png',
//       name: 'Fungicidas',
//       description:
//         'Opciones para la prevención y control de enfermedades fungosas, con perfiles preventivos/curativos y protección residual.',
//     },
//     {
//       image: '/images/Agkem_product2.png',
//       name: 'Insecticidas',
//       description:
//         'Alternativas para el manejo de insectos plaga, con diferentes modos de acción y estrategias de rotación para minimizar resistencia.',
//     },
//     {
//       image: '/images/Agkem_product3.png',
//       name: 'Herbicidas',
//       description:
//         'Soluciones para control de malezas de hoja ancha y gramíneas, con opciones selectivas y no selectivas según cultivo y momento de aplicación.',
//     },
//   ];

//   return (
//     <section className="bg-white w-full py-10 relative overflow-visible">
//       <div className="container-custom mx-auto px-4 relative">

//         {/* Title */}
//         <h2
//           className={`font-bold uppercase text-[#2C3E2C]
//           text-[26px] leading-tight mb-3
//           transition-opacity duration-500
//           ${mounted ? 'opacity-100' : 'opacity-0'}`}
//         >
//           PRODUCTOS
//         </h2>

//         {/* Intro paragraph */}
//         <p
//           className={`text-[#555] text-[17px] leading-snug max-w-[700px] mb-6
//           transition-opacity duration-700 delay-200
//           ${mounted ? 'opacity-100' : 'opacity-0'}`}
//         >
//           Nuestro portafolio abarca tres categorías principales para el manejo integral 
//           de plagas y enfermedades, con diversos modos de acción para favorecer 
//           la rotación y la eficacia.
//         </p>

//         {/* MAIN LAYOUT */}
//         <div className="relative w-full min-h-[680px]">

//           {/* LEFT COLUMN */}
//           <div className="flex flex-col w-[55%] min-w-[55%] mb-12">
//             {products.map((p, i) => (
//               <div key={i} className="mb-6">
//                 <div
//                   className="relative w-[180px] h-[70px] cursor-pointer mb-1"
//                   onClick={() => router.push(`/productos/${i + 1}`)}
//                 >
//                   <Image
//                     src={p.image}
//                     alt={p.name}
//                     fill
//                     className="object-contain"
//                     priority={i === 0}
//                   />
//                 </div>
//                 <p className="text-[#444] text-[17px] leading-snug max-w-[700px]">
//                   {p.description}
//                 </p>
//               </div>
//             ))}
//           </div>

//           {/* RIGHT COLLAGE */}
//           <div className="absolute right-0 top-0 w-[45%]">
//             <div className="relative w-full max-w-[520px] min-h-[650px]">
//               <Image
//                 src="/images/Agkem_productBG.png"
//                 alt="Products background collage"
//                 fill
//                 className="object-contain"
//               />
//             </div>
//           </div>
//         </div>

//       </div>
//     </section>
//   );
// };

// export default ProductsSection;


'use client';

import Image from 'next/image';
import { useRouter } from 'next/navigation';
import { useEffect, useState } from 'react';

const ProductsSection = () => {
  const router = useRouter();
  const [mounted, setMounted] = useState(false);

  useEffect(() => setMounted(true), []);

  const products = [
    {
      image: '/images/Agkem_product1.png',
      name: 'Fungicidas',
      description:
        'Opciones para la prevención y control de enfermedades fungosas, con perfiles preventivos/curativos y protección residual.',
    },
    {
      image: '/images/Agkem_product2.png',
      name: 'Insecticidas',
      description:
        'Alternativas para el manejo de insectos plaga, con diferentes modos de acción y estrategias de rotación para minimizar resistencia.',
    },
    {
      image: '/images/Agkem_product3.png',
      name: 'Herbicidas',
      description:
        'Soluciones para control de malezas de hoja ancha y gramíneas, con opciones selectivas y no selectivas según cultivo y momento de aplicación.',
    },
  ];

  return (
    <section className="bg-white w-full py-10 relative overflow-visible pb-0">
      <div className="container-custom mx-auto px-10">

        {/* Title */}
        <h2
          className={`font-bold uppercase text-[#2C3E2C]
          text-[26px] leading-tight mb-3
          transition-opacity duration-500
          ${mounted ? 'opacity-100' : 'opacity-0'}`}
        >
          PRODUCTOS
        </h2>

        {/* Intro */}
        <p
          className={`text-[#555] text-[17px] leading-snug max-w-[700px] mb-8
          transition-opacity duration-500 delay-200
          ${mounted ? 'opacity-100' : 'opacity-0'}`}
        >
          Nuestro portafolio abarca tres categorías principales para el manejo integral
          de plagas y enfermedades, con diversos modos de acción para favorecer
          la rotación y la eficacia.
        </p>

        {/* FLEX LAYOUT (this is the fix) */}
        <div className="flex justify-between items-start gap-6 w-full">

          {/* LEFT COLUMN */}
          <div className="w-[55%]">
            {products.map((p, i) => (
              <div key={i} className={`mb-8 transition-opacity duration-700 ${
                mounted ? 'opacity-100' : 'opacity-0'
              }`}
              style={{ transitionDelay: `${200 + i * 200}ms` }}
              >
                <div
                  className="relative w-[180px] h-[70px] cursor-pointer mb-2"
                  onClick={() => router.push(`/productos/${i + 1}`)}
                >
                  <Image
                    src={p.image}
                    alt={p.name}
                    fill
                    className="object-contain"
                    priority={i === 0}
                  />
                </div>
                <p className="text-[#444] text-[17px] leading-snug max-w-[700px]">
                  {p.description}
                </p>
              </div>
            ))}
          </div>

          {/* RIGHT COLUMN (image now aligns perfectly to top) */}
          <div className="w-[45%] flex justify-end">
            <div className={`relative w-full max-w-[520px] -top-[150px] transition-opacity duration-500 ${
              mounted ? 'opacity-100' : 'opacity-0'}`}
              style={{ transitionDelay: "600ms" }}
            >
              <Image
                src="/images/Agkem_productBG.png"
                alt="Products Collage"
                width={520}
                height={700}
                className="object-contain w-full h-auto"
              />
            </div>
          </div>

        </div>

      </div>
    </section>
  );
};

export default ProductsSection;
